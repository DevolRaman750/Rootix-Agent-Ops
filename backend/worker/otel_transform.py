...
def _extract_user_id(attrs: dict[str, Any]) -> str | None:
    ...
    return (
        attrs.get("rootix.trace.user_id") or attrs.get("user.id") or attrs.get("session.user_id")
    )


def _extract_session_id(attrs: dict[str, Any]) -> str | None:
    """Extract session_id from span attributes, checking multiple keys."""
    return attrs.get("rootix.trace.session_id") or attrs.get("session.id")
    ...
    for resource_span in resource_spans:
        ...
        for scope_span in scope_spans:
            ...
            for otel_span in otel_spans:
                ...
                # git_source_file = span_attrs.get("rootix.git.source_file")
                # git_source_line = span_attrs.get("rootix.git.source_line")
                # git_source_function = span_attrs.get("rootix.git.source_function")
                ...
                # Priority: rootix SDK attrs > OpenInference attrs
                span_input = span_attrs.get("rootix.span.input") or span_attrs.get("input.value")
                span_output = span_attrs.get("rootix.span.output") or span_attrs.get(
                    "output.value"
                )
                ...
                model_name = (
                    span_attrs.get("rootix.llm.model")
